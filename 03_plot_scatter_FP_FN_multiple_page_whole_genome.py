...
with PdfPages('{0}_FP_rcll_genome_pos_test.pdf'.format(sample_name)) as pdf:
	for WINDOWS in range(100, 200, 100):
		for FILTER in list([200, 600]):
			...
			################ WORKING ON THIS to sub sample 15 percent of the data an plot tehm##########
			# https://stackoverflow.com/questions/53543782/how-can-i-subsample-an-array-according-to-its-density-remove-frequent-values/53563514
			kde = scipy.stats.gaussian_kde(PR_haplotig_position.T)
			p = 1 / kde.pdf(PR_haplotig_position.T)
			p /= np.sum(p)
			idx = np.random.choice(np.arange(len(PR_haplotig_position)), size=100, replace=False, p=p)
			PR_haplotig_position = PR_haplotig_position[idx]
			# axes_array[axis].scatter(sample[:, 0],d, c = "darkgreen", alpha = 0.3, s = 3, marker='<',label='TP')
			...
			### Fugure main title:
			fig.suptitle('Precision-Recall FP vs FN: {0}-{1}_kb_windows_{2}_filter_components_{3}'.format((sample_name), (WINDOWS), (FILTER), (components_no)),fontweight="bold", fontsize=15)
