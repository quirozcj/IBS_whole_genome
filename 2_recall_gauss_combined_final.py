...
# filter_range = range(200, 2200, 200)                                                                            				# filter used end not inlcusive
...
# filtered_df_gauss.to_csv(sys.stdout2, index=False,  sep='\t') 
...
PR_df.rename(columns={'haplotig_count_left':'haplotig_count',
    ...
    # categorized Gaussian
    # PR_df.to_csv(sys.stdout2, index=False,  sep='\t')              # write output to the terminal original
...
# df_PR_summary.to_csv(PR_out_file_PR_summary_ind, index=False,  sep='\t')
