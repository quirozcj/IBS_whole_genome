...
with PdfPages('{0}_{1}_whole_genome_ctf_75.pdf'.format((path_file), (sample_name))) as pdf:
	# windows to capture each file
	windows = 500
	while windows <= 500:
	## create DataFrames
		count_df = pd.read_csv('{0}/whole_genome_haplo-tig_count_{1}_{2}kb_wind_ctf_75_IBS.txt'.format((path_file), (sample_name), (windows)), delimiter='\t')
		count_df_no_ibd = pd.read_csv('{0}/whole_genome_haplo-tig_count_{1}_{2}kb_wind_ctf_75_NON-IBS.txt'.format((path_file), (sample_name), (windows)), delimiter='\t')
		count_df_all = pd.read_csv('{0}/whole_genome_haplo-tig_count_{1}_{2}kb_wind_ctf_75.txt'.format((path_file), (sample_name), (windows)), delimiter='\t')
		...
		ax0.legend(prop={'size': 15})
		ax0.grid(True)
		ax0.set_title('Haplo-tig count frequency IBS vs NO-IBS_ctf_75 [{0}:whole_genome {1}kb_wind]'.format((sample_name), (windows)), fontweight="bold", fontsize=15)
